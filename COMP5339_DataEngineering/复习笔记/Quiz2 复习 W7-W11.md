...
> [!tip] 为什么出现
> 数据量超过单机处理上限 → 要么换更强的机器，要么加更多机器。两者本质区别是**有没有硬件天花板**和**有没有单点故障风险**。
...
> [!tip] 为什么出现
> 分析查询（如 `SUM(revenue) GROUP BY region`）只用 3 列，但 Row Store 读整行要扫全部 50 列——47 列的磁盘 I/O 全浪费。Column Store 只读需要的列，I/O 直接减少 94%，且同列数据类型相同，压缩率极高。
...
> [!tip] 为什么出现
> 单张表数据量太大，一台机器存不下也查不动 → 按规则把数据切成多块分散到不同节点，让查询只扫描相关分区而不是全表。
...
> [!tip] 为什么出现
> 单点存储有两个问题：① 机器宕机数据丢失；② 所有读请求压在一台机器上性能差。Replication 用多副本同时解决两个问题，但随之引入新问题：**多个副本之间的数据何时同步**——这就是 Sync vs Async 的核心矛盾。
...
> [!tip] 为什么出现
> 网络分区（P）在分布式系统中无法避免——网线会断，节点会掉包。断网时数据还没同步到所有节点，系统只能二选一：**停服等待同步**（保 C 牺牲 A）或**继续服务但可能返回旧数据**（保 A 牺牲 C）。P 是前提，C 和 A 只能选一个。
...
> [!tip] 为什么出现
> 数据分散在多台机器上 → 计算也必须分散到数据所在的机器（"让计算去找数据，而不是让数据去找计算"）。否则把 500GB 数据集中传到一台机器再处理，网络带宽立刻成瓶颈。MapReduce 解决了这个问题，但每次都读磁盘太慢 → Spark 用内存缓存解决了这个新问题。
