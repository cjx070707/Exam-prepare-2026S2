...
<span style="color: #9b59b6">💡 **白话理解**：Scale-Up 就是换台更好的电脑；Scale-Out 是买更多台普通电脑组成集群。前者简单但有上限，后者复杂但理论上无限扩。</span>
...
<span style="color: #9b59b6">💡 **白话理解**：Row Store 把一行数据存在一起，取任何数据都要读整行。Column Store 把同一列存在一起，分析查询只读用到的列——50 列只用 3 列，I/O 减少 94%。</span>
...
<span style="color: #9b59b6">💡 **白话理解**：分区就是把一张大表按规则切开，每台机器只存一部分。查询时只扫相关的那几块，不用扫全表。</span>
| 策略 | 方式 | 适用 | 代价 |
|------|------|------|------|
...
<span style="color: #9b59b6">💡 **白话理解**：把数据同时存到多台机器上。一台挂了还有别的副本可用，同时读请求可以分散到多台，提升性能。</span>
...
<span style="color: #9b59b6">💡 **白话理解**：P（网络分区）是无法避免的现实。断网时各副本数据不同步，系统只能选：等所有副本同步好再响应（保 C），还是先响应但可能给旧数据（保 A）。</span>
分布式系统只能同时满足三个中的**两个**：
...
<span style="color: #9b59b6">💡 **白话理解**：一个"目录"（NameNode）记录所有文件在哪，多个"仓库"（DataNode）实际存数据。文件切成 64MB 的块，每块存 3 份在不同机器上，某台挂了数据还在。</span>- 1 个 NameNode（元数据管理）+ 多个 DataNode（实际存储）
...
<span style="color: #9b59b6">💡 **白话理解**：MapReduce 每次算完都把结果写回磁盘，下次再从磁盘读。Spark 把中间结果留在内存，下一步直接用——迭代计算（如 ML 训练要跑几百轮）因此快 10-15 倍。</span>
...
<span style="color: #9b59b6">💡 **白话理解**：把大任务切成小任务分给多台机器各自处理（Map），然后把各台机器的结果汇总合并（Reduce）。数据在哪台机器，计算就在哪台机器跑——避免把数据传来传去。</span>
| 阶段 | 说明 |
|------|------|
...
<span style="color: #9b59b6">💡 **白话理解**：Spark 把流数据切成一小批一小批处理（微批），每批有延迟。Flink 每条数据到来立刻处理，算子之间直接传递，没有"攒一批"的过程——所以延迟更低，适合对实时性要求极高的场景。</span>
...
> [!info] 问题链
> 分布式系统解决了"算不动"的问题，但生产中的数据管道每天在跑——怎么知道数据有没有出错？→ **DataOps** 负责监控编排。ML 模型要消费数据——训练和推理时 feature 如果算法不一致，模型效果直接崩 → **Feature Store** 统一计算。Feature Store 只存最新值，审计时无法还原历史决策 → **Point-in-Time Correctness**。

---

### 数据架构类型

> [!tip] 流处理和批处理都有了，该怎么把它们组合起来——选 Lambda 还是 Kappa？
...
<span style="color: #9b59b6">💡 **白话理解**：Lambda = 两条流水线并行（批处理给准确结果，流处理给低延迟结果）；Kappa = 只用一条流水线，更简洁，但大规模历史重算效率远不如专用批处理。</span>
...
<span style="color: #e67e22">*架构选好、管道跑起来后——怎么知道数据有没有出问题？某张表 36 小时没更新，有人发现吗？*</span>

### DataOps

> [!tip] 管道里某张表 36 小时没更新，你怎么发现？
...
<span style="color: #9b59b6">💡 **白话理解**：DataOps 就是给数据管道装监控摄像头——数据没更新（Freshness）、数量异常（Volume）、字段变了（Schema），系统自动报警，不用人工盯着。</span>

---

<span style="color: #e67e22">*管道稳了，ML 模型要来取数据——3TB 数据导不出来怎么跑 ML？就算导出来了，训练用 Python 算 feature、推理用 Java 重写，逻辑稍有不同，模型效果就会崩。*</span>

### Scaling ML：两种路线

> [!tip] 数据在数据库里、模型在外面——把数据搬出来 vs 把算法搬进去，哪个更合理？
...
<span style="color: #9b59b6">💡 **白话理解**：MADlib = 算法搬进数据库跑（数据不动，避免 OOM）；Feature Store = 提前算好 feature 存起来，训练读离线历史，推理做毫秒级在线查询。</span>

---

<span style="color: #e67e22">*MADlib 解决了数据太大导不出的问题，但不能解决训练和推理 feature 计算逻辑不一致——这才是 Feature Store 存在的原因。*</span>

### Feature Store

> [!tip] 同一个 feature，训练时 Python 算，推理时 Java 重写——为什么模型效果会下降？
...
<span style="color: #9b59b6">💡 **白话理解**：Feature Store 是 feature 的统一仓库：一套定义，训练从 Offline 读历史值，推理从 Online 毫秒级查询——彻底消灭训练和推理的不一致。Point-in-Time Correctness 保证拿到的是决策当时的值，而非被后来数据覆盖的最新值。</span>
