...
> [!info] 问题链
> **静态模型处理不了序列**（无记忆、固定输入长度）→ RNN 加入隐藏状态 $h_t$（记忆）→ 但 BPTT 中梯度连乘导致消失 → LSTM 用 gate 让乘法因子可学习 → 但依然无法并行 → Self-Attention 让任意两个位置直接交互 → Transformer 完全替代 RNN → BERT（双向理解）/ GPT（因果生成）→ 图像切 patch 即序列 → ViT → $O(N^2)$ 复杂度瓶颈 → SSM/Mamba 线性复杂度替代
