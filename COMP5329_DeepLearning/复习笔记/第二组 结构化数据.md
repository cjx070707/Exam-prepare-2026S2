...
> [!info] 问题链
> **MLP 忽略图像空间结构**（参数爆炸 + 对位置无感）→ CNN 引入局部连接 + 权重共享，编入 inductive bias → 但 CNN 只能处理规则网格 → **GNN** 把聚合操作推广到任意图结构 → naive 聚合有量纲问题 → 加自环 + 对称归一化（Kipf GCN）
...
<span style="color: #e67e22">*这些组件各自解决了一个问题，但它们合在一起的力量来自一个更深的设计哲学——CNN 把"局部性"和"平移等变性"直接编进了结构，而 MLP 没有。这就是 inductive bias。*</span>
