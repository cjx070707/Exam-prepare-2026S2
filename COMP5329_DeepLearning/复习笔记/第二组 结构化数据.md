...
> [!info] 问题链
> **MLP 忽略图像空间结构**（参数爆炸 + 对位置无感）→ CNN 引入局部连接 + 权重共享 → 简单叠加层数反而更差（退化问题）→ **ResNet** skip connection 提供梯度直路 → CNN 编入 inductive bias（局部性 + 平移不变性）→ 但 CNN 只能处理规则网格 → **GNN** 把聚合操作推广到任意图结构 → naive 聚合有量纲问题 → 加自环 + 对称归一化（Kipf GCN）
...
<span style="color: #e67e22">*MLP 的两个缺陷——参数爆炸和对空间结构视而不见——指向了同一个解决方向：利用图像的局部性，让同一组权重在所有位置重复使用。*</span>
...
> [!tip] 给定输入 32×32，filter 5×5，padding 2，stride 2——在纸上算出输出尺寸。如果去掉 padding，输出尺寸会变成多大？weight sharing 把参数量从多少降到了多少？
...
<span style="color: #9b59b6">💡 **白话理解**：卷积就是用一个"放大镜"（filter）在图片上滑动，每次只看一小块区域，记录"这里有没有我要找的特征"。同一个放大镜在所有位置共用——一只猫探测器，不管猫在哪都能用。</span>
...
<span style="color: #9b59b6">💡 **白话理解**：Dilated conv 就像戴上了一副"间距可调的眼镜"——看的范围变大了，但眼睛的数量（参数）没变。适合需要大感受野但不想加参数的场景，比如语义分割。</span>

<span style="color: #e67e22">*卷积操作本身只是一层的运算。真正让 CNN 强大的是几个配套组件——池化降维、1×1 降通道、感受野扩展——它们组合在一起，让网络能在有限参数下捕捉多尺度特征。*</span>
...
<span style="color: #9b59b6">💡 **白话理解**：Pooling 就是"缩略图"操作——把一块区域压缩成一个值，保留最重要的信息（Max）或平均信息（Avg），同时让特征对小幅位移不敏感。</span>
...
<span style="color: #9b59b6">💡 **白话理解**：1×1 卷积就是在每个空间位置做一次"通道混合"——不看周围，只看当前位置的所有通道，用一个线性变换重新组合。代价极低，但能大幅降低后续的计算量。</span>
...
增大感受野的方法：
- 堆叠更多卷积层（**VGG 等价性**：2 层 3×3 = 1 层 5×5 的感受野，3 层 3×3 = 1 层 7×7，但参数更少且有更多非线性）
...
<span style="color: #9b59b6">💡 **白话理解**：Group Conv 就是把工作分组——每组只负责一部分通道，减少通道间的"全连接"。Depthwise Conv 是极端情况：每个通道完全独立，再配合 1×1 conv 做通道融合，大幅节省计算。</span>

### ResNet：解决深层网络退化问题 `★★★`

> [!tip] 你训练了 18 层和 56 层的普通 CNN，发现 56 层在训练集上的表现比 18 层更差——这不是过拟合，因为训练损失也更高。这叫什么问题？ResNet 的 skip connection 从梯度传播的角度解决了什么？Bottleneck 的三层结构是什么？

**问题（退化 Degradation）**：简单增加层数，网络性能反而下降——连训练损失都更高，说明不是过拟合，而是深层网络**更难优化**。一个假设：额外的层如果恰好是恒等映射，至少不应该变差；但优化器学不到恒等映射。

**解决方案：残差连接（Skip / Shortcut Connection）**：

$$H(x) = F(x) + x$$

$$\text{网络实际学的是残差 } F(x) = H(x) - x \text{，而不是直接学 } H(x)$$

**为什么有效（三个角度）**：
1. **梯度路径**：梯度可以沿 $x$ 这条"恒等路径"直接反传，不经过非线性层，避免消失
2. **学习目标更简单**：若最优解接近恒等映射，学 $F(x) \approx 0$ 比从零学 $H(x) = x$ 容易
3. **实用深度**：ResNet 可训练 100+ 层，远超普通 CNN 的极限

**Bottleneck 设计**（ResNet-50+）：
$$1\times1 \text{ 降维} \to 3\times3 \text{ 卷积} \to 1\times1 \text{ 升维，加 skip connection}$$

先用 $1\times1$ 降 channel 数（例如 256→64），做 $3\times3$ 卷积，再用 $1\times1$ 升回来（64→256）。中间的 $3\times3$ 操作在低维空间进行，大幅节省参数。

| 架构 | 深度可行性 | 核心创新 |
|------|---------|---------|
| Plain CNN（VGG 等）| 30+ 层退化 | — |
| ResNet | 100+ 层有效 | 残差连接 + 恒等路径 |

> **考试提示**：ResNet 是 W7 Quick Review 的第一个话题（表示它是序列建模课程的前置知识），说明考试假设你已经理解 ResNet。常考：(1) 退化问题是什么、为什么不是过拟合；(2) skip connection 对梯度有什么好处；(3) Bottleneck 结构是什么。

<span style="color: #9b59b6">💡 **白话理解**：ResNet 的 skip connection 是"开一条高速公路"——梯度可以从最后一层直接沿高速公路开回第一层，不用在每个路口减速（非线性层）。网络本身不是在学"如何变换 x"，而是在学"x 还需要额外调整什么（残差）"——这比从零学要简单得多，就像改稿比从零写更容易。</span>

<span style="color: #e67e22">*ResNet 和各种 CNN 组件都很强大，但它们共享一个更深的设计哲学——CNN 把"局部性"和"平移等变性"直接编进了结构，而 MLP 没有。这就是 inductive bias。*</span>

---

## Part 4 — CNN vs MLP：Inductive Bias `★★★`

> [!tip] 一家医疗公司要识别 X 光片中的病变区域，另一家公司要预测社交网络中的用户流失——各自适合 CNN 还是 GNN？从 inductive bias 的角度解释：CNN 的假设在 X 光片上为什么成立，在社交网络上为什么不成立。
...
<span style="color: #9b59b6">💡 **白话理解**：Inductive bias 就是"预设的世界观"。CNN 预设"相邻像素相关、特征在哪里都一样"，这在自然图像上几乎总是对的，所以小数据就能学好。没有这个预设（比如 ViT）就需要大数据自己从头摸索。</span>

<span style="color: #e67e22">*CNN 的 inductive bias 在图像上非常有效，但有一个硬性前提：输入必须是规则的网格。社交网络、分子结构、知识图谱——这些数据根本没有"网格"这个概念，CNN 无从下手。*</span>
...
<span style="color: #e67e22">*定义了"图卷积要解决什么"之后，Kipf 的 GCN 给出了最实用的答案——三步推导，每步解决一个独立的设计问题。*</span>
...
> [!tip] 你直接用邻接矩阵 $A$ 乘以节点特征 $H$，发现度数高的节点特征值异常大，而且每个节点只聚合了邻居，忽略了自身——GCN 的三步修复各自针对哪个独立的问题？为什么用对称归一化而不是 row-wise 归一化？
...
<span style="color: #9b59b6">💡 **白话理解**：GCN 就是让每个节点"听取邻居的意见"来更新自己的看法。加自环是"也听自己的"；对称归一化是"朋友多的人每条建议要打折，否则 TA 的声音会淹没所有人"；对称（而非 row-wise）是为了保持矩阵的对称性和谱性质。</span>
...
<span style="color: #9b59b6">💡 **白话理解**：谱域 GCN 是从"图上的频率"角度理解卷积——低频对应平滑变化，高频对应剧烈变化。但特征分解代价极高，而且学到的 filter 无法迁移到新图上。Spatial GCN 是它的实用化简化版。</span>
...
<span style="color: #9b59b6">💡 **白话理解**：CNN 和 GNN 都在做同一件事——聚合"邻居"信息更新自己。区别是 CNN 的"邻居"是固定的空间位置，GNN 的"邻居"是图上连接的节点。本质上都是"消息传递"，只是定义在不同的拓扑结构上。</span>
...
- [ ] 说出 VGG 等价性：2 层 3×3 = 1 层 5×5 感受野，为什么更优（参数少 + 非线性多）
- [ ] 解释 ResNet 退化问题：为什么更深的普通 CNN 训练损失更高（不是过拟合）
- [ ] 解释 ResNet skip connection 对梯度传播的好处，写出残差块公式
