...
> [!info] 问题链
> **线性模型无法分 XOR**（多层线性叠加仍是线性）→ 加隐藏层 + 非线性激活（MLP）→ 需要训练权重 → **Backprop** 用链式法则传梯度 → 梯度方向有了，走法需要优化 → **Optimizer**（Momentum / Adam）加速收敛 → 模型开始过拟合 → **Regularization**（L1/L2 / Dropout / BN）约束泛化
...
<span style="color: #e67e22">*梯度有了方向，但最简单的"沿梯度反方向走一步"在复杂的损失面上效率很低——会在峡谷里来回震荡，或者在鞍点停滞不前。Optimizer 解决的是"怎么走"的问题。*</span>
